	...
	public String approveDeny(int leaveId, int empMgrId, String status, String mgrComment) {
		...
		if (empMgrId==mgr) {
			if (status.toUpperCase().equals("YES")) {
				...
				String cmd = "Update leave_history SET LEA_MGR_COMMENTS=?, LEA_STATUS=? WHERE LEA_ID=?";
				jdbcTemplate.update(cmd, new Object[] {mgrComment,lt.toString(),leaveId});
...
//		Connection con = ConnectionHelper.getConnection();
		...
		String cmd = "INSERT INTO leave_history(LEA_START_DATE,LEA_END_DATE,LEA_NO_OF_DAYS,LEA_REASON"
				+ ",LEA_TYPE,EMP_ID,LEA_STATUS) VALUES(?,?,?,?,?,?,?)";
